...
async function main() {
  console.log('Starting SEO update script...');
  for (const update of updates) {
    try {
      // Find the article by old title
      const article = await prisma.article.findFirst({
        where: { title: update.oldTitle }
      });
      ...
      if (article) {
        ...
        console.log(`⚠️ Not found: ${update.oldTitle}`);
      }
    } catch (error) {
      console.error(`❌ Error updating ${update.oldTitle}:`, error);
    }
  }
