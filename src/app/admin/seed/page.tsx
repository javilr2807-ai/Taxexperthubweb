...
const articles = [
  ...
  { title: "Do You Pay Taxes When Moving Crypto Between Wallets?", category: "Crypto & Investment Tax", excerpt: "Find out if transferring cryptocurrency between your own wallets or exchanges triggers a taxable event." },
  { title: "Do You Pay Taxes on Crypto if You Never Sell?", category: "Crypto & Investment Tax", excerpt: "Learn about the tax rules for holding cryptocurrency and when you might owe taxes without selling your assets." },
  { title: "How Are Crypto Losses Taxed?", category: "Crypto & Investment Tax", excerpt: "Discover how to report cryptocurrency losses on your taxes to offset gains and reduce your overall tax liability." },
  { title: "What Happens If You Don't Report Crypto Gains?", category: "Crypto & Investment Tax", excerpt: "Understand the risks, penalties, and potential IRS audits if you fail to report your cryptocurrency earnings." },
  { title: "Are Stablecoins Taxable?", category: "Crypto & Investment Tax", excerpt: "Learn how the IRS taxes stablecoin transactions, including earning interest, trading, and spending." },
  { title: "How Are Dividends Taxed?", category: "Crypto & Investment Tax", excerpt: "A guide to understanding the difference between qualified and ordinary dividends and their respective tax rates." },
  { title: "Long-Term vs Short-Term Capital Gains Tax", category: "Crypto & Investment Tax", excerpt: "Compare the tax rates for long-term and short-term capital gains to optimize your investment strategy." },
  { title: "Do You Pay Taxes on ETFs?", category: "Crypto & Investment Tax", excerpt: "Learn how Exchange-Traded Funds (ETFs) are taxed, including capital gains and dividend distributions." },
  { title: "How Does Tax Loss Harvesting Work?", category: "Crypto & Investment Tax", excerpt: "Discover how to use tax-loss harvesting to sell losing investments and minimize your overall tax burden." },
  { title: "What Investment Income Is Tax-Free?", category: "Crypto & Investment Tax", excerpt: "Explore investment strategies that generate tax-free income, such as municipal bonds and Roth IRAs." },
...
async function seedArticles() {
    ...
    try {
      await prisma.article.create({
        data: {
          ...
          content: '',
          published: false,
          publishDate: new Date(startDate + interval * i),
        },
      });
      inserted++;
    } catch {
  ...
  redirect(`/admin/seed?ok=${inserted}&err=${errors}`);
}

export default async function SeedPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  const sp = await searchParams;
  const ok = sp.ok ? Number(sp.ok) : null;
  const err = sp.err ? Number(sp.err) : null;

  return (
    <div className="p-8">
      <h1 className="mb-6 text-2xl font-bold text-navy">Seed Articles</h1>
      {ok !== null && (
        <div className="mb-6 rounded-lg border border-green-300 bg-green-50 p-4 text-green-800">
          Inserted: <strong>{ok}</strong> articles. Errors: <strong>{err}</strong>
        </div>
      )}
      <p className="mb-4 text-muted-foreground">
        This will insert {articles.length} draft articles (empty content) into the database.
        Duplicate slugs will be skipped.
      </p>
      <form action={seedArticles}>
        <button
          type="submit"
          className="rounded-lg bg-navy px-6 py-3 text-sm font-semibold text-white hover:bg-navy-soft"
        >
          Seed {articles.length} Draft Articles
        </button>
      </form>
    </div>
  );
