  ...
  for (const update of updates) {
    try {
      const article = await prisma.article.findFirst({
        where: {
          OR: [
            ...
            { title: update.newTitle } // In case it's already updated
          ]
        }
      });
      ...
      if (article) {
        ...
        results.push(`Updated: ${update.oldTitle}`);
      } else {
        results.push(`Not found: ${update.oldTitle}`);
      }
    } catch (e: any) {
      errors.push(`Error on ${update.oldTitle}: ${e.message}`);
    }
  }

  return NextResponse.json({
    message: "SEO Update Completed",
    results,
    errors
  });
}
